...
omics_preparation <- function(
  ...
  if (!is.null(df_tr_updated)) {
    ...
    for (c in 2:ncol(df_tr_updated)) {
      ...
      write_xlsx(df_tr_updated_5, paste0(transc_dir_name, "/Transc_Patient_", as.character(colnames(df_tr_updated)[c]), ".xlsx"))
    }
    message("Done!")
  }
  ...
  if (!is.null(df_pr_updated)) {
    ...
    for (i in 3:length(df_pr_updated)) {
      ...
      patients = patients %>%
        mutate(significant = ifelse(difference >= 1.96 | difference <= -1.96, "+", NA)) %>%
        mutate(logpval = 2 * pnorm(as.numeric(difference), lower.tail = FALSE))
      write_xlsx(patients, paste0(prot_dir_name, "/Prot_Patient_", as.character(colnames(df_pr_updated)[i]), ".xlsx"))
    }
    message("Done!")
  }
  ...
  if (!is.null(df_ph_updated)) {
    ...
    for (i in 6:length(df_ph_updated)) {
      ...
      patients = patients %>%
        mutate(significant = ifelse(difference >= 1.96 | difference <= -1.96, "+", NA))
      write_xlsx(patients, paste0(phospho_dir_name, "/Phospho_Patient_", as.character(colnames(df_ph_updated)[i]), ".xlsx"))
    }
    message("Done!")
  }
}

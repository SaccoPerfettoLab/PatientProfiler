...
omics_preparation <- function(
  ...
  if (!is.null(df_ph_updated)) {
    ...
    # Patients' values start after n° patients - n° columns reporting phosphosites information
    ...
    for (i in start_pos:length(df_ph_updated)) {
      pt1 <- df_ph_updated[, 1:length(column_order)]
      patients = cbind(pt1, df_ph_updated[, i])
      colnames(patients)[1:(start_pos)] = c(column_order, "difference")
      patients = patients %>%

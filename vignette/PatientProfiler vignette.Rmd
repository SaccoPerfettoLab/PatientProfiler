...
⚠️ Warning: The order of the columns is important and must strictly follow this structure. Incorrect column ordering may lead to errors during data processing.
...
2.  Format the input in the way described in the "Prepare your data"
    section;
3.  Choose the percent *threshold* of NAs to filter rows with \>= threshold% of
    missing values;
4.  Choose the **number of imputations** for proteomics and
    phosphoproteomics;
5.  Choose **how to pull** dataframes derived from the imputation of proteomics and
    phosphoproteomics;
6.  If you provide a phosphoproteomics dataframe, choose the *lenght of
    the sequence window*. You can insert **7** or **15;**
7.  Choose the **imputation** method for proteomics and
    phosphoproteomics;
8.  If you want to **normalize** with Z-score your data, insert *TRUE*
    for the zscore parameter, if not choose *FALSE;*
9.  In the case you select *TRUE* for the Z-score calculation, you can
...
Furthermore, it's possible to performs Kaplan-Meier survival analysis using the so obtained Signatures.
To do this you need: a list of gene signatures (e.g., those extracted from Step 5), a transcriptomic expression matrix and optionally a clinical data table with patient survival information (serial_time, status)


### [*test_signature_on_dataset*]{style="color: #274472;"}

### [Example of usage:]{style="color: #C38370;"}

```{r test_signature_example, eval = FALSE}

# Define gene_signatures
gene_signatures <- list(
  community_1 = c(
    "ARHGDIB", "STARD3", "RAPGEFL1", "ERBB2", "FAM83E"
  ),
  community_2 = c(
    "CRABP1", "GABRP", "ELF5", "S100A1", "SNORD36A"
  ),
  community_3 = c(
    "TPRG1", "CHAD", "MALRD1", "BMPR1B", "CST9", "ADCY1", "RGS22", "DNAJC12", "LINC00993",
    "DNALI1", "LONRF2", "MIR6835"
  )
)
...
transc <- t_omic_TCGA %>% dplyr::rename( gene_name = Name)
colnames(transc)[-1] <- gsub("\\.", "-", colnames(transc)[-1])



clinical <- read_tsv("clinical_TCGA_example.tsv")

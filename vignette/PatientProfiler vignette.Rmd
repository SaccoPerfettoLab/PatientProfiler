...
![](images/PatientProfiler-visual_selection.png)

## <span style="color: #41729F;">PREPARE YOUR DATA</span>
...
## <span style="color: #A45C40;"> Install PatientProfiler</span>
...
## <span style="color: #41729F;">STEP 1) RAW DATA HARMONIZATION</span>
...
###<span style="color: #C38370;">Example of usage: </span>

```{r chunk2, eval = FALSE}
transcriptomics_data <- read_tsv("./PatientProfiler_raw_input/Ov_transc.tsv") 
proteomics_data <- read_tsv("./PatientProfiler_raw_input/Ov_prot.tsv") 
phosphoproteomics_data <- read_tsv("./PatientProfiler_raw_input/Ov_phospho.tsv")  
...
## <span style="color: #41729F;">ACCESS TO PRE-HARMONIZED CPTAC DATA</span>
...
## <span style="color: #41729F;">INPUT TABLES TO INFER PROTEIN ACTIVITY AND NETWORK CONSTRUCTION</span>
...
### <span style="color: #274472;">*omics_preparation*</span>
...
tr_updated <- read_xlsx("PatientProfiler_processed_input/Transcriptomics_updated.xlsx")
pr_updated <- read_xlsx("PatientProfiler_processed_input/Proteomics_updated.xlsx")
ph_updated <- read_xlsx("PatientProfiler_processed_input/Phosphoproteomics_updated.xlsx")
...
## <span style="color: #41729F;">STEP 2) PROTEIN ACTIVITY INFERENCE</span>

## <span style="color: #050A30;">*Infer signaling proteins activity at single patient level*</span>

### <span style="color: #274472;">*extract_protein_activity*</span>
...
extract_protein_activity(
                prot_file = paste0("Prot_patients/Prot_Patient_", patient_id, ".xlsx"),       
                trans_file = paste0("Transc_patients/Transc_Patient_", patient_id, ".xlsx"),   
                phospho_file = paste0("Phospho_patients/Phospho_Patient_", patient_id, ".xlsx"), 
                tf_params,
                kin_params,
...
For each patient an .xlsx file (`Activity_patient_{patient_id}.xlsx`)
...
## <span style="color: #050A30;">*Infer signaling proteins activity at cohort level*</span>

### <span style="color: #274472;">*extract_cohort_activity*</span>
...
```{r chunk6, eval = FALSE}
tf_params <- list(reg_minsize = 5, collectri = TRUE)
kin_params <- list(reg_minsize = 10, exp_sign = TRUE)
...
The cohort final results will be saved as a set of .xlsx files (1 file -
1 patient=) inside the `output_dir` directory (default, 'Activities').
Each patient-specific file (`Activity_patient_{patient_id}.xlsx`)
...
## <span style="color: #41729F;">STEP 3) NETWORK GENERATION</span>

## <span style="color: #050A30;">**Create a signaling network for a single patient**</span>
...
### <span style="color: #274472;">*create_network*</span>
...
mutations_df <- readr::read_delim('./PatientProfiler_raw_input/Ov_mutations.csv',delim = ',')
sources <- mutations_df[mutations_df$Patient_ID == patient_id, ]

# Read activity file (intermediate nodes of the model)
activities <- readxl::read_xlsx(paste0('./Activities/Activity_Patient_', patient_id, '.xlsx'))

# Set the functional traits vector (final nodes of the model)
desired_phenotypes <- c('APOPTOSIS', 'PROLIFERATION')

# Read omics file
proteomics <- readxl::read_xlsx(paste0('./Prot_patients/Prot_Patient_', patient_id, '.xlsx'))
transcriptomics <- readxl::read_xlsx(paste0('./Transc_patients/Transc_Patient_', patient_id, '.xlsx'))
phosphoproteomics <- readxl::read_xlsx(paste0('./Phospho_patients/Phospho_Patient_', patient_id, '.xlsx'))
...
## <span style="color: #050A30;">*Create signaling networks at cohort level*</span>

### <span style="color: #274472;">*create_cohort_network*</span>
...
create_cohort_networks(trans_dir = './Transc_patients/',
                      ...
                      phospho_dir = './Phospho_patients/',
                      act_dir = './Activities/',
                      mut_file = './PatientProfiler_raw_input/Ov_mutations.csv',
                      output_dir = './Networks_output/',
                      desired_phenotypes = c('APOPTOSIS', 'PROLIFERATION'),

...
mutations_df <- read_tsv('./PatientProfiler_raw_input/Brca_mutations.tsv')
sources <- mutations_df[mutations_df$Patient_ID == patient_id, ]

# Read activity file (intermediate nodes of the model)
activities <- read_tsv(paste0('./Activities/Activity_Patient_', patient_id, '.tsv'))

# Set the functional traits vector (final nodes of the model)
desired_phenotypes <- c('APOPTOSIS', 'PROLIFERATION')

# Read omics file
proteomics <- read_tsv(paste0('./Prot_patients/Prot_Patient_', patient_id, '.tsv'))
transcriptomics <- read_tsv(paste0('./Transc_patients/Transc_Patient_', patient_id, '.tsv'))
phosphoproteomics <- read_tsv(paste0('./Phospho_patients/Phospho_Patient_', patient_id, '.tsv'))
